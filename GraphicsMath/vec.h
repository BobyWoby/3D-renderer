...
namespace gm {
    ...
    template <> struct vec<2, double> {
        public:
            ...
            double dot(vec<2, double> other) { return x * other.x + y * other.y; }
    };
    ...
    template <> struct vec<3, double> {
        public:
            ...
            vec<3, double> operator*(const double &scalar) {
                return vec<3, double>(x * scalar, y * scalar, z * scalar);
            }
            vec<3, double> operator/(const double &scalar) {
                return vec<3, double>(x / scalar, y / scalar, z / scalar);
            }
    ...
    template <> struct vec<4, double> {
        public:
            double x, y, z, w;
            vec<4, double>(double x, double y, double z, double w)
                : x(x), y(y), z(z), w(w) {}
            double &operator[](int index) {
                switch (index) {
    ...
    struct Matrix {
        public:
            ...
            Matrix(int rows, int cols) : rows(rows), cols(cols) { alloc(); }

            Matrix(double *init, int rows, int cols) : rows(rows), cols(cols) {
                alloc();
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        data[i][j] = *(init + i * cols + j);
                    }
                }
            }
            ...
            Matrix(const Matrix &other)
                : Matrix(other.data, other.rows, other.cols) {} // copy constructor
                                                                
            Matrix(Matrix &&other) noexcept
                : data(std::exchange(other.data, nullptr)), rows(other.rows),
                cols(other.cols) {}
            ...
            Matrix inverse() {
                ...
                // cofactor(i, j) = -1^(i + j) * det(reduce(A))
                if (rows != cols) {
                ...
                for (int i = 0; i < this->rows; i++) {
                    for (int j = 0; j < this->cols; j++) {
                        ...
                        double cofactor = std::pow(-1, i + j) * determinant(this->reduce(i, j));
                        // std::cout << "cofactor at (" << i << ", " << j << "): " << cofactor << std::endl;
            ...
            static double determinant(Matrix in) {
                if (in.rows != in.cols) {
                    std::cerr << "Can't find determinant of a non-square matrix!"
                        << std::endl;
                    return 0;
                }
                ...
                for (int i = 0; i < in.cols; i++) {
                    ...
                    // std::cout << "std::pow(-1, " << i << "): " << std::pow(-1, i) <<
                    // std::endl; std::cout << "gm::Matrix::determinant(in.reduce(0, " << i <<
                    // ")): " << gm::Matrix::determinant(in.reduce(0, i)) << std::endl;

                    sum += in.data[0][i] * std::pow(-1, i) *
                        gm::Matrix::determinant(in.reduce(0, i));
                }
                return sum;
            }
            ...
            Matrix operator*(vec3d vec) {
                if(this->cols != 3){
                    std::cerr << "Not a valid matrix to multiply with a vec3d";    
                }
                assert(this->cols == 3);
